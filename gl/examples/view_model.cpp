...
void window_surface(gl_context& context)
{
...
void app_surface(gl_context& context)
{
    gl_window(context, { .size = { .width = 200, .height = 200 } }, "View model example")();
}

int main(int argc, char** argv)
{
    gl_context context;
    context.register_application(gl_application("ru.zuma.view_model", app_surface, context));
    context.run();
}